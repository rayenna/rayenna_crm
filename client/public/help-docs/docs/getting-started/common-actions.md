...
**Quick open**: **Ctrl+Shift+N** (Windows/Linux) or **⌘⇧N** (Mac), or open **`/customers?new=1`** in the browser after login — same **New Customer** form as below (role limits apply).
...
- **CRM keyboard shortcuts** (when not typing in a field): **?** opens Help; **Ctrl+Shift+D** Dashboard, **C** Customers, **P** Projects, **K** Support Tickets, **Z** Zenith; **Ctrl+Shift+M** opens **My Day** (CRM-linked tasks, journal, reminders — see [Zenith → My Day](/help/zenith#my-day-personal-productivity-drawer)); **Ctrl+Shift+N** opens **Customers** with **New Customer** (same as **`/customers?new=1`**); **E** **new project** (role limits apply). On Mac use **⌘** instead of **Ctrl**. Full list: [Getting Started → Keyboard shortcuts](/help/getting-started#keyboard-shortcuts) or [Dashboard → Keyboard shortcuts](/help/dashboard#keyboard-shortcuts).
