...
export const TIPS: string[] = [
  ...
  'Zenith Your Focus sits below the Deal flow funnel in Pipeline and priorities: Sales see pipeline with Deal Health and Log activity plus Proposal Engine rows (click a PE bucket → Quick Actions list → Open in Projects); Finance see Payment radar (ageing buckets, Top overdue, Remind for WhatsApp/Email); Operations see Installation pulse (progress, + Log update, horizontal scroll on phones); Management and Admin see all panels including Proposal Engine.',
  'Zenith Payment radar: click a payment ageing band (0–30 through 90+ days) to filter the Top overdue table; under Top overdue, the hint explains project name (open project / payment work) vs Remind; use Remind for prefilled WhatsApp or Email — your app sends the message, not Rayenna.',
  ...
  'Zenith tables: click a column header to sort (e.g. Deal value, Health, Last activity). Use Filter customer…, All stages, and All salespeople where shown — including Today’s Hit List (up to seven urgent deals) and Your pipeline today under Your Focus.',
  ...
  'Zenith: Today’s Hit List shows up to seven urgent deals with the same filter/sort pattern as Company pipeline today, plus an Alert column — Open → opens Quick Actions; when deals list, a short hint may remind you to scroll right and use Open → on narrow screens.',
  'Zenith Quick Actions (single project): scroll to Recent remarks before Log activity — same remark history as Project detail, newest first, read-only in the drawer; use Open full project for the full list and edits.',
