...
export const TIPS: string[] = [
  ...
  'Zenith Your Focus sits between KPIs and the funnel: Sales see pipeline with Deal Health and Log activity; Finance see Payment radar (ageing buckets, Top overdue, Remind for WhatsApp/Email); Operations see Installation pulse (progress, + Log update, horizontal scroll on phones); Management and Admin see all three.',
  ...
  'Zenith The Board: pick Month, Quarter (Indian FY), or FY on the card — rankings use stage-entered / confirmation dates so periods can show different totals. Click header totals or a salesperson’s value to open the same Quick Actions list drawer as Explore charts.',
