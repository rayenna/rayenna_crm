...
export const TIPS: string[] = [
  ...
  'Create Support Tickets from the Support Tickets page (pick a project) or from project detail. Set a next follow-up date so Overdue stays honest.',
  ...
  'Add follow-ups to Support Tickets with a next date (defaults to +3 days). Pin overdue tickets to My Day from the ticket drawer.',
  // Revenue & Pipeline
  ...
  'Projects: Needs review flags mismatched dates, payments, or system size. Saving with commissioning before confirmation, or advance above order value, asks you to confirm first.',
  ...
  'Deal Health “Deal value” peaks for order values in the ₹2L–₹8L sweet spot; above ₹8L there is only a mild point discount — see Help → Projects → Deal Health Score for the full table.',
  'Deal Health “Commitment” folds confirmation, advance, payment/collection status, and expected commissioning (overdue costs points). Activity uses the latest remark, My Day, payment, or update. Help → Projects → Deal Health Score.',
  ...
  'Today’s plan is on Zenith (Today / KPIs), not the classic Dashboard — My Day teasers plus Suggested from CRM with + My Day. On Sales / Admin / Management desktop it sits beside the Hit List; on mobile KPIs stay first. Sales and Admin may also see Things needing attention when late-stage brands are missing; Sales, Admin, and Management see Needs review for date/payment mismatches. Finance sees the plan only. Full Tasks / Journal / Reminders stay in the nav My Day drawer.',
  'Zenith Payment radar: click a payment ageing band (0–30 through 90+ days) to filter Top overdue; Latest payments received lists recent receipts beside Top overdue on wide layouts; Sl No. on both tables; click a project name to open the Finance quick drawer when that path is wired; the legend under the tables explains name colours by payment status; use Remind (or Draft payment reminder in the Finance drawer / Project Detail) for an editable WhatsApp or Email draft — your app sends the message, not Rayenna; you can log a project remark when you open it.',
  ...
  'Deal Health badges appear in Zenith Pipeline and Hit List — hover a badge to see what’s hurting the score (activity, momentum, value, commitment, source).',
  ...
  'Zenith on a phone: use the bottom tabs (Today, Pipeline, Charts, More on Finance) — one section at a time instead of one endless scroll. Floating ✦ Briefing and back-to-top sit above the tab bar.',
  'Zenith offline (installed PWA): after you have loaded Zenith online, you may see saved dashboard data with a gold banner when the API is unreachable. Log activity and other allowed drawer edits queue until you tap Sync or reconnect.',
  'Zenith command bar shows Live when the CRM API responds and Offline when it does not — more reliable on mobile than the browser alone. My Day still needs a live connection.',
  'Classic Dashboard: the scrolling banner under the title promotes My Day (Ctrl+Shift+M or the ☀ icon) and Zenith under the Dashboard menu.',
  ...
  'My Day Journal: one personal note per day. Type freely — it auto-saves after a short pause. Recent entries appear below; use Load more for older notes. Expand any entry to read the full note.',
  'My Day tasks: use the ⋯ menu on a row to Edit or Delete — not swipe gestures. Reminders with any past due date stay in Overdue until you complete them.',
