...
- **Solar News** — A scrolling ribbon of **industry headlines** from **RSS feeds** aggregated on the server (no HTML scraping in the browser). Shown **above** **AI Insights** for every role that opens Zenith.
- **AI Insights** — A scrolling ribbon of short, plain-English highlights from your **CRM data** already loaded for your filters (**below** Solar News, still directly under the command bar).
- **The Board** — Sales leaderboard with **clickable totals and rows** that open drill-down lists.
- **Your Focus** — **Collapsible** panels tailored to your **role** (e.g. pipeline table with **Sl No.** / **Prj #**, **Payment radar** with **Top overdue**, **Latest payments received**, and shared **payment status** legend where applicable, installation pulse, **Proposal Engine** readiness rows for **Sales / Management / Admin** where applicable).
...
From **Zenith**, press **?** (when not typing in a field) to open **Help** on this **Analytics** page and jump to **Zenith Command Center**. A **Smart daily briefing** may appear when you land on Zenith — see [Daily briefing](#daily-briefing-zenith).
