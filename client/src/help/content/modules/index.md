...
- **[Zenith ✦](/help/analytics#zenith-command-center)** — full-screen command center (same filters), **The Board** leaderboard (Month / Quarter / FY + clickable drill-down lists), **collapsible Your Focus**, **Today’s Hit List** (pipeline-style **filters + sort** on up to seven urgent deals + optional **scroll right / Open →** hint + **Quick Actions**), **Revenue forecast**, **Explore** chart drill-downs, **Availing Loan** KPI → **Quick Actions** list (Finance + executive Zenith), **Payment radar** **Top overdue** helper text (**project name** vs **Remind**), **Victory** toast on winning stage changes, and **Quick Actions** lists (**Recent remarks** above **Log activity** in single-project drawers); deep dive under Analytics → Zenith Command Center
