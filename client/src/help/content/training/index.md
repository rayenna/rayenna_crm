...
| Use case | Suggestion |
| :-- | :-- |
| **Live workshop (60–90 min)** | Follow **Modules 1–8** for Sales; use **Module 9** as breakout or appendix. |
...
4. Use **Your Focus → Your pipeline today** to prioritise deals, **Deal Health**, and **Log activity** (Sales).
5. **Explore** charts under **Explore the landscape**, open the **Quick Actions** drawer in **list** mode, and open a **single project** for quick updates **when their role allows**.
6. **Recognise** what **Management** and other roles see differently (company-wide vs own pipeline, view-only drawer).
...
| E | Your Focus (pipeline / payment radar / installation) | ✓ Pipeline | Role panels |
| F | Deal flow funnel | ✓ | ✓ |
| G | Explore charts & drill-down | ✓ Deep | ✓ |
| H | Quick Actions drawer & permissions | ✓ Deep | View vs edit |
| I | Role-specific summary | — | ✓ |
...
- **Clickable analytics**: many charts open a **Quick Actions** side drawer with a **filtered project list** (and optional quick edits for permitted roles).
...
2. Encourage users to set **filters first**, then read KPIs → Focus → Funnel → Explore.
...
- **Click an insight** → the page **scrolls** to a related block (KPIs, Your Focus, funnel, charts).
...
# Module 5 — Your Focus: Your pipeline today (Sales)

**Location:** Between KPIs / forecast and the **funnel**.
...
# Module 6 — Deal flow funnel

- Visual **stage mix** from leads through execution (exact tiles depend on role).
...
# Module 7 — Explore the landscape (charts and drill-down)
...
- **Proposal Engine** card (where your role has it on the classic dashboard)
...
2. Click **Open →** on one row → **single-project** drawer (see Module 8).
3. Use **Back** to return to the list without losing the chart context.

---

# Module 8 — Quick Actions drawer (list and single project)
...
# Module 9 — Zenith for other roles (summary)
...
- **Company-wide** executive KPIs and **all** Your Focus blocks (Sales pipeline + Finance radar + Operations installation) in one scroll.
...
| 4 | In **Your pipeline today**, sort by **Deal Health** | Order updates |
| 5 | Drill from **Projects by stage** | Drawer list count looks plausible |
| 6 | Open one project from list, **log activity** (if your project) | Toast / success; remark visible on project |
| 7 | **Reset** filters | KPIs reflect broader scope |
| 8 | Optional: `Ctrl+Shift+Z` from Dashboard | Zenith opens |

**Facilitator:** For **Management** trainees, repeat steps 1–5 only; confirm step 6 **cannot** edit in drawer (view-only).
...
| **Drill-down** | From chart → drawer **list** of projects behind that slice. |
