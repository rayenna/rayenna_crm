/** Keep in sync with `src/utils/mapGpsValidation.ts` (CRM backend). */
...
export function getKeralaMapGpsWarning(latitude: number, longitude: number): string | null {
  ...
  return (
    ...
    `~76.2°–77.4°E. Fix Map GPS in Customer Master or paste a corrected Google Maps URL below, then Regenerate.`
  );
}
