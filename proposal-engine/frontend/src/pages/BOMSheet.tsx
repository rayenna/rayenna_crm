...
import {
  BOM_FROM_COSTING_KEY, ROI_AUTOFILL_KEY,
...
function loadOverrides(): BomOverrides | null {
  try {
    const raw = localStorage.getItem(BOM_OVERRIDES_KEY);
...
function persistOverrides(sheetId: string, rows: BomRow[]) {
  ...
  localStorage.setItem(BOM_OVERRIDES_KEY, JSON.stringify(data));
}

function loadStoredBom(): StoredBom | null {
  try {
    const raw = localStorage.getItem(BOM_FROM_COSTING_KEY);
    return raw ? JSON.parse(raw) : null;
  } catch { return null; }
}
...
export default function BOMSheet() {
  ...
  const onSubmit = useCallback((data: FormValues) => {
    ...
    try {
      ...
      try {
        const existing = localStorage.getItem(ROI_AUTOFILL_KEY);
        if (existing) {
          ...
          localStorage.setItem(ROI_AUTOFILL_KEY, JSON.stringify(updated));
        }
      } catch { /* ignore */ }
  ...
  const handleRegenerateFromCosting = useCallback(() => {
    ...
    localStorage.removeItem(BOM_OVERRIDES_KEY);
    const rows = (stored.rows as Partial<BomRow>[]).map(normalizeRow);
    initialRowsRef.current = rows;
    reset({ rows });
    setSource('costing-sheet');
