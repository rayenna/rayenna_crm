...
function normalizeRow(r: Partial<BomRow>): BomRow {
  const hasGst = Object.prototype.hasOwnProperty.call(r, 'gstPercent');
  return {
    ...
    brand:           r.brand ?? '',
    // Only carry gstPercent through when it exists on the source row.
    gstPercent:      hasGst ? (r.gstPercent ?? '') : undefined,
...
function GstSummaryPanel({ rows }: { rows: BomRow[] }) {
  // Classify each item by explicit GST %, falling back to keyword-based inference
  // only when no gstPercent field exists at all.
  const getRate = (r: BomRow): number => {
    if (r.gstPercent !== undefined) {
      const v = parseFloat(r.gstPercent);
      return !Number.isNaN(v) && v >= 0 ? v : 0;
    }
    return deriveGstRate(r.itemName ?? '');
  };
  const at5  = rows.filter((r) => getRate(r) === 5);
  const at18 = rows.filter((r) => getRate(r) === 18);
...
function BomRowInput({
  ...
  const gstPercentRaw = useWatch({ control, name: `rows.${index}.gstPercent` }) as string | undefined;
  const gstRate  = (() => {
    // If this row has a gstPercent field (i.e. came from Costing Sheet), always respect it.
    if (gstPercentRaw !== undefined) {
      const v = parseFloat(gstPercentRaw ?? '');
      return !Number.isNaN(v) && v >= 0 ? v : 0;
    }
    // Rows without gstPercent (older/manual BOMs) fall back to keyword-based inference.
    return deriveGstRate(itemName ?? '');
  })();
