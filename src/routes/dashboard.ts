...
async function loadZenithExplorerProjects(where: Prisma.ProjectWhereInput) {
  const rows = await prisma.project.findMany({
    ...
    select: {
      ...
      grossProfit: true,
      projectRemarks: { orderBy: { createdAt: 'desc' as const }, take: 1, select: { createdAt: true } },
      availingLoan: true,
      financingBank: true,
      salespersonId: true,
  ...
  return rows.map((p) => ({
    ...
    advance_received: p.advanceReceived != null ? Number(p.advanceReceived) : 0,
    balance_amount: p.balanceAmount != null ? Number(p.balanceAmount) : null,
    expected_close_date: p.expectedCommissioningDate?.toISOString() ?? null,
    last_remark_at: p.projectRemarks[0]?.createdAt?.toISOString() ?? null,
    ...
    const buildSalesPipeline = async (scope: 'self' | 'all') => {
      ...
      const rows = projects.map((p) => {
        ...
        return {
          ...
          lastRemarkAt: p.projectRemarks[0]?.createdAt?.toISOString() ?? null,
