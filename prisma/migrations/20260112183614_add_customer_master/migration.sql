...
-- CreateIndex: Create unique index on customerId
...
BEGIN
    ...
    LOOP
        ...
        ELSE
            ...
            -- Insert new customer
            -- Generate a unique ID (using a simple approach compatible with all PostgreSQL versions)
            customer_db_id := md5(random()::text || clock_timestamp()::text) || substr(md5(random()::text || clock_timestamp()::text), 1, 8);
        ...
        WHERE "projects"."customerName" = project_record."customerName"
          ...
          AND "projects"."customerId" IS NULL;
    END LOOP;
END $$;
